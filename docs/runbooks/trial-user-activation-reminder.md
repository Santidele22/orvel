...
1. Confirm alignment with `pnpm dlx supabase@2.98.2 migration list`, then run `scripts/trial-reminder-production.sh preflight legacy-applied`. This read-only intermediate gate requires the exact historical schema from `20260710210000`, no generic helper/normalizer, and either zero rows or one migratable `reserved` row with null `finalized_at`. Stop on schema drift, terminal/inconsistent state, or multiple rows.
...
6. After a zero exit, re-run `pnpm dlx supabase@2.98.2 migration list`; require `20260712213000` exactly once and aligned. Finally run `scripts/trial-reminder-production.sh preflight present`. Continue only when both verification commands exit zero. Never run the `legacy-applied` gate after the forward migration.

## Stages

1. **Review:** `preflight pristine` is only for a new database before either reminder migration. For production, run checked-in `prerequisites`, then `preflight legacy-applied` immediately before the forward migration and `preflight present` immediately after it; stop on any non-zero result.
