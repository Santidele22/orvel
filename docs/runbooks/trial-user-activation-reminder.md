...
- The ACL migration first removes applicable global and `public`-schema default function EXECUTE for PUBLIC, anon, authenticated, and service_role under the verified function owner. It then removes direct EXECUTE from those roles on all four legacy functions and grants only reserve/finalize to service_role. Its transactional postcondition checks direct, effective, and default access and rolls back on owner, security, membership, default-ACL, or direct-ACL mismatch. Use the checked-in `trial-reminder-function-acl-diagnostic.sql` only for read-only troubleshooting; its output is role/function allowlisted and identity-free.
...
Run only `pnpm run trial-reminder:production forward-migrate`. Do not run the Bash script or `supabase db push` directly. With both migrations pending, the composite stage verifies the exact diagnosed `legacy-acl-drift` state, requires exactly ACL then generic in the dry-run, performs a bounded push without `--include-all`, verifies aligned history, and runs the present gate. If the ACL migration is already applied and only generic remains, it first requires the clean legacy schema/ACL/default gate, requires exactly the generic migration in the dry-run, then pushes and present-checks. Generic-applied-without-ACL and any extra migration are impossible states and fail closed. No independent migration stage is exposed.

Each migration file commits independently; do not claim cross-file atomicity. On timeout or any non-zero result, stop and do not continue to secrets, deployment, or invocation. If the ACL migration committed while the generic migration failed, the composite reports the recoverable ACL-applied/generic-pending history state. Re-run only `forward-migrate`: it verifies clean legacy ACL/default state, requires exactly one pending generic migration, and fixes forward under the same CLI timeout. Never invoke during recovery, rerun an invocation, terminate an unknown backend, or run migration repair.
