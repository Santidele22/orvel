...
- The ACL migration records the exact distinct relevant owner set from the reminder table, all four legacy functions, and the actual migration actor that will create future helpers. For every owner it removes global and `public`-schema default function EXECUTE for PUBLIC, anon, authenticated, and service_role; the global revoke explicitly removes PostgreSQL's intrinsic PUBLIC EXECUTE even when no global `pg_default_acl` row existed. It fails closed unless the actor can alter every owner and rejects an unknown fourth owner. It then removes direct EXECUTE from those roles on all four legacy functions and grants only reserve/finalize to service_role. Transactional postconditions verify the recorded owner set plus direct, effective, and default access. The generic migration independently verifies the actor/default closure before helper creation, reasserts all six function ACLs, and validates the complete owner/default/effective matrix before commit. Use the checked-in diagnostic only for read-only troubleshooting; owner identities are emitted only as allowlisted categories.
