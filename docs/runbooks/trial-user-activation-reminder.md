...
- The ACL migration derives the exact deduplicated owner set from the reminder table, the exact four legacy functions, and the actual migration actor that will create future helpers. Legitimate overlap naturally yields one through six owners; no unrelated role is included. For every derived owner it removes global and `public`-schema default function EXECUTE for PUBLIC, anon, authenticated, and service_role; the global revoke explicitly removes PostgreSQL's intrinsic PUBLIC EXECUTE even when no global `pg_default_acl` row existed. It fails closed if an object or owner cannot be resolved or the actor cannot alter every derived owner's defaults. It then removes direct EXECUTE from those roles on all four legacy functions and grants only reserve/finalize to service_role. Transactional postconditions verify the recorded owner set plus direct, effective, and default access. The generic migration and all read-only gates derive that same set, independently verify its ACL/default closure before helper creation, reassert all six function ACLs, and validate the complete owner/default/effective matrix before commit. Use the checked-in diagnostic only for read-only troubleshooting; owner identities are emitted only as allowlisted categories.
...
- `prepare-and-invoke` requires the 14-digit migrations `20260712190000` and `20260712213000` to appear exactly once, in order, with aligned local/remote history. The parser accepts only the pinned Supabase CLI 2.98.2 fixed-width three-column table (`Local`, `Remote`, `Time (UTC)`) and versions of exactly 8 or 14 digits. Unrelated legacy 8-digit rows are accepted only when already applied and aligned; they cannot be pending or duplicated. Truly blank remote cells are accepted only for the two expected 14-digit recovery migrations. Malformed headers, separators, cells, order, duplicates, mismatches, and extra pending rows fail closed. It fails before invocation if either expected migration is absent or the durable row already exists.
