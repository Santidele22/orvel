...
import { computeAvailableSlots } from '../domain/availability/availability-core';
import type { WeekdayKey, WorkingDayHours } from '../facades/business-settings.facade';
import type { NotificationServicePort } from './notification.service';
...
import {
  ...
  type AdminCancelBookingPayload,
  type AdminRescheduleBookingPayload,
  type AdminStatusUpdatePayload
} from '../core/api/supabase-booking.api';
...
export class TurnoService {
  ...
  private async createWithSupabase(dto: CreateTurnoDTO, startsAtIso: string): Promise<Turno> {
    ...
    // Call Supabase RPC
    const response = await createAdminManualBooking(payload);
  ...
  private updateWithMock(id: string, dto: UpdateTurnoDTO): Observable<Turno> {
    ...
    if (index === -1) {
      return throwError(() => new Error('TURNO_NOT_FOUND'));
    }
  ...
  private async updateWithSupabase(id: string, dto: UpdateTurnoDTO): Promise<Turno> {
    ...
    if (dto.clienteId || dto.servicioId || dto.duracionMinutos !== undefined) {
      ...
      const response = await updateAdminBooking(payload);
  ...
  private async cancelByAdminWithSupabase(id: string, payload: AdminActionPayload): Promise<Turno> {
    ...
    const response = await cancelAdminBooking(payloadSupabase);
  ...
  private async rescheduleByAdminWithSupabase(id: string, payload: AdminReschedulePayload): Promise<Turno> {
    ...
    const response = await rescheduleAdminBooking(payloadSupabase);
