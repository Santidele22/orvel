import type { PlanCode } from '../../plans/plan-entitlements';
  ...
  const maybeWindow = globalThis as {
    __supabase__?: {
      functions?: {
        invoke?: (
          fnName: string,
          options: { body: { plan_code: PlanCode } }
        ) => Promise<InvokeResult>;
      };
    };
  };

  const invoke = maybeWindow.__supabase__?.functions?.invoke;

  if (!invoke) {
    throw new CreateSubscriptionError(
      'SERVER_CONFIG_ERROR',
      'No pudimos inicializar el cliente de pagos. Contactá soporte.'
    );
  }

  return (payload) => invoke('create-subscription', { body: payload });
}
