...
const PLAN_ORDER: readonly CanonicalPlanCode[] = CANONICAL_PLAN_CODES;
...
const PLAN_COPY: Record<CanonicalPlanCode, { name: string; priceMonthlyCents: number; billingCadences: { monthly: number; quarterly: number; annual: number } }> = {
  ...
  STARTER: { 
    ...
    billingCadences: { 
      monthly: 12, 
      quarterly: Math.round(12 * 3 * 0.85), 
      annual: Math.round(12 * 12 * 0.70) 
    } 
  },
  GROWTH: { 
    name: 'Growth', 
    ...
    billingCadences: { 
      monthly: 22, 
      quarterly: Math.round(22 * 3 * 0.85), 
      annual: Math.round(22 * 12 * 0.70) 
    } 
  },
  PRO: { 
    name: 'Pro', 
    ...
    billingCadences: { 
      monthly: 39, 
      quarterly: Math.round(39 * 3 * 0.85), 
      annual: Math.round(39 * 12 * 0.70) 
    } 
  }
};
...
function fromPlanEntitlementsRows(rows: PlanEntitlementRow[]): LandingPlanViewModel[] {
  ...
  return PLAN_ORDER.map((code) => byCode.get(code) ?? fallbackByCode.get(code)).filter(
