-- Public booking must not fail closed when the business has no support/owner email.
-- Availability already shows slots without that recipient, so confirm must insert
-- the booking and dashboard notification. Customer email remains optional.
...
BEGIN
  ...
  -- This RPC does not enqueue a business-owner email. Keep the flag honest.
