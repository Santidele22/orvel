-- Service deposit percent: seña is 0/25/50/100 of the selected service price.
-- Does not edit WU1/WU2. Hold amount no longer uses business_settings.deposit_amount_pesos.
...
ALTER TABLE public.services
  DROP CONSTRAINT IF EXISTS services_deposit_percent_check;
...
BEGIN
  ...
  SELECT s.duration_minutes, s.name, s.price, COALESCE(s.deposit_percent, 0)
    INTO v_duration_minutes, v_service_name, v_service_price, v_deposit_percent
  ...
  IF v_deposit_enabled AND COALESCE(v_deposit_percent, 0) > 0 THEN
