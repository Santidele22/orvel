-- P0 security fix: booking management bearers must be hash-only at rest.
-- Public manage URLs remain compatible because create_public_booking still
-- returns the one-time plaintext bearer to the caller for immediate response
-- and notification payload composition, but plaintext is never written to
-- public.bookings by this forward contract.
...
SET manage_token_hash = public._hash_manage_token(manage_token)
