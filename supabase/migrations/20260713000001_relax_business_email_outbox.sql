-- Relax BUSINESS_EMAIL_OUTBOX_REQUIRED — business email outbox is no longer
-- a hard constraint for ordinary booking operations.
-- Phase 2 of 3-phase migration (must deploy AFTER harden_dashboard_notifications_required).
--
-- Changes:
-- 1. create_public_booking: remove business email INSERT (appointment_created_business),
--    the v_business_email_rows counter, and the BUSINESS_EMAIL_OUTBOX_REQUIRED check.
-- 2. handle_booking_notifications: remove business email INSERT (booking_created_business).

BEGIN;

-- ---------------------------------------------------------------------------
-- create_public_booking — remove business email outbox constraint
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- dashboard_notifications — required (harden from fase 8)
...
-- ---------------------------------------------------------------------------
-- handle_booking_notifications — remove business email outbox
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT name INTO v_business_name FROM public.businesses WHERE id::text = NEW.business_id::text;
