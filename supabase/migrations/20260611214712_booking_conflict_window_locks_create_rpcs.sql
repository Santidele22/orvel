-- Serialize booking creation conflict checks by business/time bucket to prevent
-- concurrent count-before-insert races in public and admin create paths.

BEGIN;

CREATE OR REPLACE FUNCTION public._lock_booking_conflict_window(
  p_business_id uuid,
  p_branch_id uuid,
...
BEGIN
  ...
  -- Normalize [starts_at, ends_at) into inclusive 30-minute bucket numbers.
  -- The branch id is intentionally not part of the key: locking is business-scoped
  -- so branch-wide and all-branch conflict checks serialize on the same buckets.
  v_bucket_start := floor(extract(epoch FROM p_starts_at) / 1800)::bigint;
  v_bucket_end := floor((extract(epoch FROM p_ends_at) - 0.000001) / 1800)::bigint;

  FOR v_lock_key IN
    SELECT hashtextextended(
      'orvel.booking.conflict_window:' || p_business_id::text || ':' || bucket::text,
      0
    )
    FROM generate_series(v_bucket_start, v_bucket_end) AS buckets(bucket)
    ORDER BY 1
  LOOP
...
REVOKE ALL ON FUNCTION public._lock_booking_conflict_window(uuid, uuid, timestamptz, timestamptz) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._lock_booking_conflict_window(uuid, uuid, timestamptz, timestamptz) FROM anon;
REVOKE ALL ON FUNCTION public._lock_booking_conflict_window(uuid, uuid, timestamptz, timestamptz) FROM authenticated;
...
BEGIN
  ...
  PERFORM public._lock_booking_conflict_window(create_admin_manual_booking.business_id, create_admin_manual_booking.branch_id, v_starts_at, v_ends_at);
  ...
  INSERT INTO public.bookings (business_id, branch_id, customer_id, service_id, starts_at, ends_at, status, manage_token, professional_id, created_by, notes, source)
  VALUES (create_admin_manual_booking.business_id, create_admin_manual_booking.branch_id, v_customer_id, v_service_id::text, v_starts_at, v_ends_at, 'confirmed', v_manage_token, v_professional_id, create_admin_manual_booking.performed_by, NULLIF(btrim(create_admin_manual_booking.notes), ''), 'admin-manual')
  RETURNING id INTO v_booking_id;
