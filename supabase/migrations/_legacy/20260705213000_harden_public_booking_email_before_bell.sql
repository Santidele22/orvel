-- Harden public booking success so the dashboard card contract and business email fallback
-- are owned by the database. Public callers must use an existing active principal
-- branch; tenant branch creation/repair remains outside the anon booking RPC.
-- Bell notification failures must not roll back a booking once the business-owner
-- email outbox row has been safely enqueued.
...
BEGIN
  ...
  IF v_customer_email IS NOT NULL THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'Orvel public booking customer email skipped after business email enqueue: booking %, business %, sqlstate %, message %', v_booking_id, v_business_id, SQLSTATE, SQLERRM;
    END;
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE LOG 'Orvel public booking bell notification skipped after business email enqueue: booking %, business %, sqlstate %, message %', v_booking_id, v_business_id, SQLSTATE, SQLERRM;
  END;
