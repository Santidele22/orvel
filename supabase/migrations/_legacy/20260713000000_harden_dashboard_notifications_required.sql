-- Harden dashboard_notifications as required for booking lifecycle.
-- Phase 1 of 3-phase migration (must deploy before relax_business_email_outbox).
--
-- Changes:
-- 1. create_public_booking: remove EXCEPTION wrapper around dashboard_notifications
--    INSERT so that a failure propagates and reverts the transaction.
-- 2. handle_booking_notifications: remove ON CONFLICT DO NOTHING and move the
--    dashboard_notifications INSERT before outbox email enqueues.

BEGIN;

-- ---------------------------------------------------------------------------
-- create_public_booking — make dashboard_notifications atomic
-- ---------------------------------------------------------------------------
...
DECLARE
  ...
  v_business_email_rows integer := 0;
...
BEGIN
  ...
  -- business email outbox (still required — relaxed in fase 9)
  ...
  GET DIAGNOSTICS v_business_email_rows = ROW_COUNT;

  IF v_business_email_rows < 1 THEN
    PERFORM public._raise_rpc('BUSINESS_EMAIL_OUTBOX_REQUIRED');
  END IF;
  ...
  -- dashboard_notifications — now REQUIRED: failure here reverts the whole transaction
...
-- ---------------------------------------------------------------------------
-- handle_booking_notifications — remove ON CONFLICT DO NOTHING, move
-- dashboard_notifications BEFORE outbox
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- dashboard_notifications FIRST (atomic — failure reverts the whole operation)
  INSERT INTO public.dashboard_notifications (business_id, appointment_id, event_type, title, body, metadata)
  ...
  -- business email outbox (still present — relaxed in fase 9)
