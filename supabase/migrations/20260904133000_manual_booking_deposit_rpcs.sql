-- Manual booking deposits (WU2): public create hold path + confirm/claim RPCs.
-- Does not edit 20260904120000 (already applied on QA). Legacy public.bookings only.
...
CREATE OR REPLACE FUNCTION public._generate_booking_deposit_code()
RETURNS text
LANGUAGE plpgsql
SET search_path = public, pg_temp
AS $$
...
REVOKE ALL ON FUNCTION public._generate_booking_deposit_code() FROM PUBLIC;
...
BEGIN
  ...
  v_deposit_amount := (v_config->>'deposit_amount_pesos')::numeric;
  ...
  IF v_deposit_enabled THEN
    IF v_deposit_amount IS NULL OR v_deposit_amount <= 0 OR (v_deposit_alias IS NULL AND v_deposit_cbu IS NULL) THEN
...
REVOKE ALL ON FUNCTION public.claim_booking_deposit(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.claim_booking_deposit(text, text) TO anon, authenticated;
