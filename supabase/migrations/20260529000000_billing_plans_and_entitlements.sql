...
as $$
  select
    ...
    bs.tenant_id::text,
    bs.plan_code,
    coalesce(bs.subscription_status, bs.status) as subscription_status,
    pe.max_locales,
    pe.max_rubros,
  ...
  where bs.business_id::text = $1
    and bs.tenant_id::text = $2
    and coalesce(bs.subscription_status, bs.status) in ('active', 'trialing')
  limit 1;
$$;
