// release-expired-booking-holds Edge Function
// Secure scheduler/manual entrypoint for release_expired_booking_hold(NULL, NULL).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabase.rpc("release_expired_booking_hold", {
      p_booking_id: null,
      p_business_id: null,
    });
    ...
    if (error) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "RELEASE_EXPIRED_BOOKING_HOLDS_FAILED",
        }),
        { status: 500, headers: jsonHeaders },
      );
    }
