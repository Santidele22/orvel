// operator-lifecycle-pushes Edge Function
// Secure scheduler/manual entrypoint for enqueue_operator_lifecycle_pushes().
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabase.rpc("enqueue_operator_lifecycle_pushes");

    if (error) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "OPERATOR_LIFECYCLE_PUSHES_FAILED",
        }),
        { status: 500, headers: jsonHeaders },
      );
    }
