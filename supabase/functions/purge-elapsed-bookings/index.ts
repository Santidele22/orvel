// purge-elapsed-bookings Edge Function
// Secure scheduler/manual entrypoint for purge_elapsed_bookings.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("purge_elapsed_bookings");

    if (error) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "PURGE_ELAPSED_BOOKINGS_FAILED",
        }),
        { status: 500, headers: jsonHeaders },
      );
    }
