// signup-email-verification-lifecycle Edge Function
// Secure scheduler entrypoint for enqueue_signup_email_verification_actions.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("enqueue_signup_email_verification_actions");

    if (error) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "SIGNUP_EMAIL_VERIFICATION_ENQUEUE_FAILED",
        }),
        { status: 500, headers: jsonHeaders },
      );
    }
