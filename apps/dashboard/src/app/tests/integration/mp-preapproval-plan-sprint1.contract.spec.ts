...
const SPRINT1_MIGRATION = path.join(ROOT, 'supabase', 'migrations', '20260507_mp_preapproval_plan_sprint1.sql');
...
describe('Sprint1 MP preapproval_plan validation contracts', () => {
  it('supports create-subscription with new {tier,cadence} model and keeps legacy plan_code fallback', () => {
    ...
    expect(source).toMatch(/effectivePlanCode\s*=\s*normalizedTier\s*===\s*["']started["'][\s\S]*["']STARTER["'][\s\S]*["']GROWTH["'][\s\S]*["']PRO["']/m);
    expect(source).toMatch(/const canonicalPlanCode = normalizePlanCode\(effectivePlanCode\)/);
  ...
  it('persists external references and MP ids on pending subscription insert', () => {
    const source = readRequiredFile(CREATE_SUBSCRIPTION_FN);

    expect(source).toMatch(/\.from\(["']business_subscriptions["']\)[\s\S]*\.insert\(\{[\s\S]*mp_preapproval_id\s*:\s*mpData\.id[\s\S]*mp_external_reference\s*:\s*externalReference[\s\S]*mp_init_point\s*:\s*mpData\.init_point[\s\S]*\}\)/m);
  });
  ...
  it('keeps webhook idempotency intact after dual-write additions', () => {
    ...
    const readExisting = source.search(/\.from\("payment_webhook_events"\)[\s\S]*\.select\("id, processed_at, payload_hash"\)/m);
