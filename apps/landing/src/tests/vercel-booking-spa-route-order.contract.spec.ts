...
function firstMatchingDest(routes: Array<Record<string, unknown>>, pathname: string) {
  ...
  for (const route of afterFilesystem) {
    ...
    const src = route.src.startsWith('^') ? route.src : `^${route.src}$`;
    if (new RegExp(src).test(pathname)) {
      return route.dest;
    }
  }
  return undefined;
...
describe('Contract: Vercel SPA rewrite stays after filesystem and before 404', () => {
  ...
  it('keeps /booking/manage on the SPA rewrite dest, not a 404 dest', () => {
    ...
    expect(dest).not.toBe(BOOKING_SHARE.dest);
  });

  it('places tenant booking-share dest before the /booking SPA rewrite', () => {
    const fixture = {
      version: 3,
      routes: [{ handle: 'filesystem' }, NOT_FOUND],
    };
    ...
    const patched = patchVercelOutputConfig(fixture);
    const routes = patched.routes as Array<Record<string, unknown>>;
    const shareIndex = routeIndex(
      routes,
      (route) => route.src === BOOKING_SHARE.src && route.dest === BOOKING_SHARE.dest,
    );
    const bookingIndex = routeIndex(
      routes,
      (route) => route.src === BOOKING_SPA.src && route.dest === BOOKING_SPA.dest,
    ...
    expect(shareIndex).toBeLessThan(bookingIndex);
    expect(firstMatchingDest(routes, '/booking/nails-nora')).toBe(BOOKING_SHARE.dest);
    expect(firstMatchingDest(routes, '/booking/manage')).not.toBe(BOOKING_SHARE.dest);
  });

  it('keeps /booking with no slug on the SPA rewrite after filesystem', () => {
    const fixture = {
      version: 3,
      routes: [{ handle: 'filesystem' }, NOT_FOUND],
    };
