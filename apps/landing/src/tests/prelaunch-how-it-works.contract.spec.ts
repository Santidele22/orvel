...
describe('Contract: prelaunch Cómo funciona section', () => {
  ...
  it('uses real booking and dashboard screenshots instead of HTML phone mocks', async () => {
    const source = await howItWorksSource();

    expect(source).toContain('/prelaunch/public-turnero-portal.png');
    expect(source).toContain('/prelaunch/public-turnero-form.png');
    expect(source).toContain('/prelaunch/showcase-inicio-web.png');
    expect(source).toContain('/prelaunch/showcase-inicio-mobile.jpg');
    expect(source).toContain('/prelaunch/showcase-agenda-web.png');
    expect(source).toContain('/prelaunch/showcase-agenda-mobile.jpg');
    expect(source).toContain('/prelaunch/showcase-clientes-web.png');
    expect(source).toContain('/prelaunch/showcase-clientes-mobile.jpg');
  ...
  it('auto-advances the active audience and respects reduced motion', async () => {
    ...
    expect(source).toContain('data-how-mock-step="3"');
    ...
    expect(source).toMatch(/business:\s*4/);

    const mockStepValues = [...source.matchAll(/data-how-mock-step="(\d+)"/g)].map(
      (match) => match[1]
    );
    expect(new Set(mockStepValues)).toEqual(new Set(['0', '1', '2', '3']));
    expect(mockStepValues.filter((value) => value === '0').length).toBeGreaterThanOrEqual(2);
    expect(mockStepValues.filter((value) => value === '3').length).toBeGreaterThanOrEqual(1);
