...
describe('Contract: landing login returnTo resolves to dashboard app', () => {
  ...
  it('navigates the Google login button explicitly when OAuth returns a redirect target', () => {
    const loginPage = readFileSync(resolve(process.cwd(), 'src/pages/auth/login.astro'), 'utf8');

    expect(loginPage).toMatch(/const result = await loginWithGoogle\(\{ redirectTo: callbackUrl\.toString\(\) \}\)/);
    expect(loginPage).toMatch(/if \(result\.ok && result\.redirectTo\) \{\s*window\.location\.assign\(result\.redirectTo\);\s*return;\s*\}/s);
    expect(loginPage).toMatch(/if \(result\.ok\) \{\s*setError\([^)]*no pudimos redirigirte[^)]*\);\s*button\.disabled = false;\s*return;\s*\}/is);
    expect(loginPage).toContain('debug_oauth');
    expect(loginPage).toContain('OAuth debug');
  });
