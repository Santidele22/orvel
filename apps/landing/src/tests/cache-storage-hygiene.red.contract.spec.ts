...
describe('RED contract: landing signup/subscription browser storage hygiene', () => {
  ...
  it('manual signup persists only non-sensitive temporary fields and never stores passwords', async () => {
    ...
    expect(signupSources).toMatch(/La contraseña nunca se persiste|password.*never.*storage/i);
  ...
  it('retains the subscription idempotency key during retry and clears it only on approved/materialized subscription', async () => {
    ...
    expect(approvedImmediateHandling, 'Immediate approved return must clear the retained attempt key before dashboard handoff.').toMatch(/sessionStorage\.removeItem\(subscriptionAttemptKey\)/);
  });
});
