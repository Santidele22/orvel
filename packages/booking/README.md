...
1. **Explicit shim, not `export *`** — the 3 old paths in `apps/dashboard/src/app/core/api/supabase-booking/` are thin re-export shims with explicit per-name re-exports (`export type { ... } from '@orvel/booking'`). Keeps the public surface grep-able and survives `isolatedModules`. Deletable once consumers migrate (follow-up, out of scope).
...
- 3 dashboard shims at the old paths re-export from `@orvel/booking` (deletable follow-up once consumers migrate).
